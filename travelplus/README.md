...
5. Dam bao `writable/` co quyen ghi; khong ghi de counter, log va `writable/stats/website-settings.json` dang co tren hosting.
...
Trang `/admin/website-settings` luu hotline, email va cac kenh lien he vao `writable/stats/website-settings.json`. Tinh nang nay khong can SQL, migration hoac command tren hosting.

Sau khi publish, kiem tra: trang chu VI/EN, booking lookup, form lien he, email, thanh toan, `/sitemap.xml`, `/robots.txt`, mot URL 404, mot thu muc bi chan 403, `/admin/website-settings`, `/admin/system-health` va `/admin/system-logs`.
